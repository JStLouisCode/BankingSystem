...
class BankAccount:
    ...
    def send_money(self):
        if self.login:
            contact_name = input("Who would you like to send money to?") #give two button inputs saying either yes or no
            contact_found = False
            ...
            transfer_amount = input("How much do you want to transfer") #Have a space where user can type number amount
            if transfer_amount > self.balance:
                print("Insufficient funds. ")
                time.sleep(5)
                print("Account Balance: $", self.balance)
            else:
                self.balance = self.balance - self.amount #want to have a 'send money' button
    ...
    def logout(self):
        ...
        self.login = False
        print("You have been successfully logged out.") #should i redirect back to the login page or close the application completely?
